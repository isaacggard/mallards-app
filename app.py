...
from fan_master_cleaner import (
    build_fan_master,
    classify_file,
    read_columns,
    read_csv,
...
st.set_page_config(page_title="Fan Master Builder", layout="wide")
inject_base_css()
render_sidebar_nav("builder")
...
if done:
    ...
    if invalid_files:
        ...
        try:
            ...
            csv_bytes = fan_master.to_csv(index=False).encode("utf-8")
            st.session_state["fan_master_csv"] = csv_bytes
...
if "fan_master_csv" in st.session_state:
    ...
    st.markdown(
        ...
        <div class="result-panel">
            ...
            <div class="result-copy">The dashboard page can use this file immediately from the current session.</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    with action_cols[1]:
        st.page_link("pages/1_Fan_Dashboard.py", label="Open Dashboard")
